...
export function buildStaticPrompt(
  ...
  return [
    ...
    intents,
    '',
    'HAI QUY TAC PHAN LOAI BAT BUOC — xet TRUOC khi ket luan mot tin la "mo ho":',
    '- DON VIET DANG BANG KE VAN LA DON. Mot tin co "<so luong> x <ten SP>" (hoac "<ten SP> x <so luong>") la don dat hang RO RANG -> intent=dat_don, confidence CAO. KHONG doi phai co dong tu ("gui"/"dat"/"lay"): dai ly thuong go thang mot dong bang ke chu khong viet thanh cau. Phan dau dong kieu "<Chi nhanh>_<Ngay>_<Ten dai ly>" la SIEU DU LIEU cua don (noi giao, ngay, ben mua) — no LAM RO don chu khong lam tin mo ho. TRU KHI trong tin co dau hieu ro rang cua mot y dinh khac (hoi gia, doi tra/khieu nai, hoi van chuyen) thi chon y dinh do.',
    '- MOT TOKEN LA KHONG DOI DUOC PHAN LOAI. Ma tham chieu, ma don noi bo, chuoi/so ngau nhien hay dau vet he thong deu co the lan vao mot tin that. BO QUA phan khong hieu duoc va phan loai theo PHAN CON LAI cua tin; rieng su co mat cua mot token la KHONG BAO GIO la ly do de tra ve khac.',
    '',
