  ...
  it(
    ...
    () => {
      const script = [
        ...
        "const { SettlementRepository } = await import('./src/transport/settlement/settlement.repository.ts');",
        ...
        "const { CommercialAcceptanceController } = await import('./src/transport/acceptance/commercial-acceptance.controller.ts');",
        '',
        ...
        'const settlementRepo = context.get(SettlementRepository, { strict: false });',
        '',
        ...
        '',
        '// KE TOAN ghi quyet dinh ket thuc — mot TAI KHOAN THAT trong nguon tai khoan (`#334`).',
        'const users = context.get(UserRepository, { strict: false });',
        "const accountant = await users.create({ username: 'ke-toan-boot', name: 'Ke toan Boot K', email: null, phone: null, passwordHash: 'x', role: 'ACCOUNTING' });",
        ...
        '  authUserId: accountant.id,',
        '});',
        '',
        '// `#334` — doc qua CONTROLLER THAT: hinh chieu doc phai duoc tiem va phai goi duoc ten.',
        'const controller = context.get(CommercialAcceptanceController, { strict: false });',
        'const shown = await controller.detail(projection.order.id);',
        'const listed = (await controller.queue({})).acceptances.find((row) => row.orderId === projection.order.id);',
        '',
        "let confirmedReason = 'KHONG BI CHAN';",
        "try { await settlement.recogniseCustomerReceivable(trip.id, 'boot'); }",
        'catch (error) { confirmedReason = error.reason ?? error.name; }',
        "let confirmedReplayReason = 'KHONG BI CHAN';",
        "try { await settlement.recogniseCustomerReceivable(trip.id, 'boot'); }",
        'catch (error) { confirmedReplayReason = error.reason ?? error.name; }',
        "const customerDocuments = await settlementRepo.listDocuments({ flow: 'CUSTOMER_FREIGHT' });",
        '',
        'const proof = {',
        ...
        '  decidedByIsSessionUser: decided.decisions[0].decidedBy === accountant.id,',
        '  history: {',
        '    rawKept: shown.decisions[0].decidedBy === accountant.id,',
        '    actorIdKept: shown.decisions[0].decidedByActor.id === accountant.id,',
        '    label: shown.decisions[0].decidedByActor.label,',
        '    kind: shown.decisions[0].decidedByActor.kind,',
        '  },',
        '  latest: listed ? {',
        '    rawKept: listed.latestDecidedBy === accountant.id,',
        '    label: listed.latestDecidedByActor?.label ?? null,',
        '  } : null,',
        '  confirmedReason,',
        '  confirmedReplayReason,',
      ...
      expect(JSON.parse(proof ?? '{}')).toEqual({
        ...
        state: 'APPROVED',
        // `#334`: ma tho la id PHIEN, va ca lich su lan hang cho goi duoc TEN tu nguon tai khoan.
        decidedByIsSessionUser: true,
        history: { rawKept: true, actorIdKept: true, label: 'Ke toan Boot K', kind: 'USER' },
        latest: { rawKept: true, label: 'Ke toan Boot K' },
        confirmedReason: 'SETTLEMENT_CUSTOMER_RECONCILIATION_REQUIRED',
        confirmedReplayReason: 'SETTLEMENT_CUSTOMER_RECONCILIATION_REQUIRED',
