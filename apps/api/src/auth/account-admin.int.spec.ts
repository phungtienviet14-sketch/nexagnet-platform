...
describe.runIf(process.env.RUN_PRISMA_IT === '1')('quan tri tai khoan tren Postgres THAT (#395)', () => {
  ...
  it('mat khau tam qua han → TEMPORARY_PASSWORD_EXPIRED o dang nhap, phien cu chet', async () => {
    ...
    expect(
      await service.validateSession({ userId: created.id, credentialVersion: created.credentialVersion }),
    ).toBeNull();
    // CHECK cua DB: bat doi mat khau ma khong co han thi khong ghi duoc.
    await expect(
      prisma.user.update({ where: { id: created.id }, data: { temporaryPasswordExpiresAt: null } }),
  ...
  it('dat lai mat khau KHONG mo khoa; mo khoa roi dat lai moi khoi phuc duoc', async () => {
    ...
    await expect(
      service.authenticate({ username: accountant.username, password: reset.credential.temporaryPassword }),
  ...
  it('danh sach loc tren Postgres; trung so dien thoai → ACCOUNT_IDENTITY_TAKEN', async () => {
    ...
    expect(
      await reasonOf(service.updateProfile(director, two.id, { phone: '0399395001' })),
