...
const fixtureDir = resolve(apiDir, '../../packages/tenant/src/__tests__/fixtures/knowledge-only');

describe('knowledge-only process boot contract', () => {
  it(
    'boot Nest that ma khong can Zalo, parser, dealer, gia hay order graph',
    () => {
      const script = `
