...
export interface DepotOpenWork {
  /** Vong chay `PLANNED`/`ACTIVE` co chang chua huy di tu / ve nhan bai nay. */
...
export abstract class DepotOpenWorkReader {
  abstract openWorkAt(depotLabel: string): Promise<DepotOpenWork>;
}
...
export class MovementDepotOpenWorkReader extends DepotOpenWorkReader {
  ...
  async openWorkAt(depotLabel: string): Promise<DepotOpenWork> {
