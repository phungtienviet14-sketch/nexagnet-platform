...
| 3 | Ghim gh-aw có đủ tag `vX.Y.Z` + SHA 40 hex | `.github/workflows/agent-builder.md` |
...
## Hai bài đang SKIP, và vì sao đó không phải xanh giả

Bài **6** và **9** tự bật khi điều kiện của chúng tới, không cần ai nhớ bật:
...
Cả hai đều in **lý do skip** ra output, nên một lần skip không thể bị đọc nhầm thành một lần đạt.

## Đã kiểm bằng mutation, không chỉ bằng "xanh"

Hai đột biến đã chạy thật trên HEAD này và **cả hai đều bị bắt**:

| Đột biến | Kết quả |
|---|---|
| `staged: true` → `staged: false` | bài 1 đỏ |
| `issues: read` → `issues: write` | bài 4 đỏ |
