...
không chọn/mở được chứng từ bằng chuột. Và xác nhận một chỗ chặn cũ vẫn còn: lái xe **không ghi được chi phí dọc
...
| 7   | Kết thúc đơn (ACCOUNTING)                     | ⚠️ làm được sau khi **cuộn ngang** và **gõ tay căn cứ** — chuột không chọn được chứng từ (N2)                              |
...
| N2  | Mới — có từ trước, bản nền không đo tới | `OrderCompletionView` + `ConfirmAction` (không đổi từ `e816fe82`) | **P0** | D03 — khối "Chứng từ số của đơn" chỉ render khi hộp thoại mở, nhưng render **ngoài** hộp thoại: `.tx-confirm` phủ toàn màn (`position:fixed; inset:0; z-index:50`), `elementFromPoint` tại ô chọn = `DIV.tx-confirm`. Chuột/cảm ứng không chọn, không mở được tệp chứng từ (chỉ còn đường ẩn bằng bàn phím: Shift+Tab ×2 ra khỏi hộp thoại — không có bẫy focus — rồi Space); trên thực tế căn cứ thành `EXTERNAL_PHYSICAL_CONFIRMATION` + gõ tay | **#382**    |
...
| **#382** | P0-2 + N2          | Bước kiểm soát thương mại (kết thúc đơn theo chứng từ) không dùng được chứng từ bằng chuột/cảm ứng; hành động chính không thấy                                    |
