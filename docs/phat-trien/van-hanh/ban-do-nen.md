...
hiển thị đúng trên nền đơn giản.”** — nói *nền* hỏng, không nói toạ độ sai, và không nêu tên hạ
...
- **Không một định danh nào ra ngoài.** URL style là hằng số; MapLibre chỉ gọi những URL do style
  ...
  nào gửi tới OpenFreeMap. Trình duyệt chỉ gửi `Referer` là **origin** (chính sách mặc định
  `strict-origin-when-cross-origin`), nên đường dẫn `?selected=RUN-…` của trang cũng không ra ngoài.
  Bài "sống" `@openfreemap-basemap` kiểm cả ba điều này trên trình duyệt thật.
...
3. **Khoá trình duyệt có GIỚI HẠN:** *Websites (HTTP referrers)* chỉ tên miền thật của môi trường
   (`http://localhost:<cổng>/*` chỉ cho một khoá thử riêng); *API restriction* chỉ **Maps JavaScript
...
4. **Chi phí:** mỗi lần dựng `google.maps.Map` là một *map load* (SKU Dynamic Maps) — mở màn bản đồ,
...
**Map ID quyết định cách tuyến nằm trên nền Google** (đo trên Google thật 23/09/2026: *"The map is
initialized without a valid map ID, which will prevent use of WebGLOverlayView."*):
...
- **Tuỳ chọn** (không nằm trong 7 check bắt buộc) — biến đặt trước lệnh, chạy từ `apps/web`:
  ...
  Bài OpenFreeMap "sống" đòi: nền `OPENFREEMAP`, không câu thông báo, ghi nguồn chứa *OpenFreeMap* và
  *OpenStreetMap*, cả hai màu tuyến hiện ra (đếm điểm ảnh), **mọi** yêu cầu ngoài chỉ tới
