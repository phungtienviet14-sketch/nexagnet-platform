...
2. **Outbound theo policy tenant.** Đơn GĐ1 hợp lệ trong ngưỡng được tự xác nhận theo policy đã duyệt; nội dung chiến dịch CSKH bắt buộc Sale duyệt trước khi lịch chạy. Các loại outbound dài hạn khác phải khai báo policy fail-closed, không dùng một điều kiện duyệt chung cho mọi intent. `AUTO_SEND` chỉ là kill switch vận hành.
...
| Cổng ERP + map SKU↔mã hàng số *(sau GĐ1)* | Chỉ là cổng vào cho các tính năng dài hạn cần đồng bộ ERP; không thuộc đường găng GĐ1 |
...
**Nghiệp vụ:** NetViet GĐ1 đã cam kết "Dashboard cơ bản"; `docs/khach-hang/ultty/thiet-ke-giao-dien/` của khách có tab Tổng quan (4 counter + biểu đồ theo giờ + hoạt động gần đây). Đây là món **trả nợ thiết kế**, không phải tính năng mới hoàn toàn.
...
- Quyền tự gửi đơn trong ngưỡng đã được chốt ở **D4**. Mỗi năng lực dài hạn vẫn phải có outbound policy riêng; campaign luôn cần Sale duyệt nội dung trước lịch chạy.
