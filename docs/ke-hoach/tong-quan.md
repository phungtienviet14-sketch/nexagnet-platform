...
> **Kế hoạch con:** [gd1.md](gd1.md) (**GĐ1 theo spec khách — 3 agent, đọc TRƯỚC khi làm tiếp**) · [nen-tang.md](nen-tang.md) (Đợt 0 — nền phải xong) · [tinh-nang-dai-han.md](tinh-nang-dai-han.md) (Đợt 1-4 — 6 tính năng mới).
...
- **▶️ PHIÊN SAU BẮT ĐẦU TỪ ĐÂY (bàn giao 11/08/2026).** Việc đầu tiên: **Đợt A′ — Task 1** trong [gd1.md §4 CHẶN E](gd1.md) — cho tin chỉ-ảnh đi qua pipeline. Cụ thể: `packages/shared/src/channel-message.ts` đổi `text: z.string().min(1)` → `z.string().max(10_000)` (cho phép chuỗi rỗng) + `.refine(m => m.text.trim() !== '' || !!m.imageUrl)`; bỏ `if (!text) return null` ở [bot-poller.ts:73](../../apps/api/src/ingest/bot-poller.ts:73) và [zca-message.ts:27](../../apps/api/src/ingest/zca-message.ts:27). **Giữ `text` là `string` (KHÔNG optional)** để không call-site nào phía sau vỡ kiểu — đây là điểm mấu chốt của cách làm này. Sẽ có spec đang khẳng định hành vi cũ (tin không chữ → `null`), phải sửa theo. Kiểm: `pnpm typecheck && pnpm --filter @ultty/api test`. **Lý do ưu tiên số 1:** không phụ thuộc câu trả lời nào của khách, và mỗi ngày chưa làm là mất vĩnh viễn ảnh của ngày đó.
...
> 📋 **Bản hỏi Sale (không kỹ thuật):** [docs/mau/checklist-hoi-sale.md](../mau/checklist-hoi-sale.md)
