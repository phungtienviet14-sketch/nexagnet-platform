...
- **⛔ KÊNH BOT PLATFORM CHẾT — xác nhận bằng token mới (05/08/2026).** Người vận hành cấp lại `ZALO_BOT_TOKEN`; token cũ nay trả **401 Unauthorized** (đã thu hồi thật), token mới `getMe` **200 OK** cùng bot id `4055584533866160964`. **Nhưng `getUpdates` vẫn 504** — thử `{timeout:20|5|1}` và body rỗng, cả POST lẫn GET, từ máy local lẫn từ VM: **lần nào cũng 504 ở đúng ~5,13 giây** (nginx của Zalo bỏ cuộc chờ upstream), tham số `timeout` không có tác dụng nào. `getWebhookInfo` trả **404** ⇒ Bot Platform **không có đường webhook** để thay long-polling. ⇒ Kết luận: **sự cố phía Zalo, không có cách sửa bằng code, không có đường vòng.** Xoay token là **cần** (giữ `botIdentity` xanh trong console, tránh 401) nhưng **không đủ**. Hệ quả vận hành: `CHANNEL_MODE=hybrid` vẫn bật nhưng **thực chất chỉ zca đọc được**; tag bot trong nhóm không có phản hồi; `BotPoller` ghi cảnh báo 504 mỗi ~8 giây (ồn log, vô hại). Nếu cần log sạch thì hạ về `CHANNEL_MODE=zca` — `render-secrets.sh` tự làm điều đó khi secret token trống. Secret đã lên version 2; VM đã render lại + deploy 05/08.
