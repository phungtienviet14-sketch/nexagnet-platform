...
4. 4️⃣ Pháp lý/nguồn gốc — **để nguyên, không sửa**. Từ 11/08/2026 gom theo khách tại [docs/khach-hang/](docs/khach-hang/README.md) (mỗi khách một thư mục `nguon-goc/` · `trao-doi/` · `design-app/`; hiện có **ultty** + **amico**). Của Ultty: `docs/khach-hang/ultty/nguon-goc/khao-sat-khach-hang-2026-07.docx` (hồ sơ khảo sát gốc) · `docs/khach-hang/ultty/nguon-goc/ho-so-khao-sat/` (quy trình + PO + bảng giá — gitignored vì PII) · `docs/khach-hang/ultty/nguon-goc/de-xuat-giai-phap-netviet.md` (đề xuất giải pháp NetViet — GIỮ NGUYÊN, không sửa file này)
5. Tài liệu phụ trợ: `docs/khach-hang/amico/nguon-goc/bao-gia-ai-agent-2026-08.md` (báo giá khách thứ 2 — 4 mâu thuẫn cần chốt, xem [docs/ke-hoach/nen-tang-da-khach.md](docs/ke-hoach/nen-tang-da-khach.md) §8) · `docs/khach-hang/ultty/design-app/` (8 ảnh design app của khách — tham khảo UX) · `docs/pdf/` (**3 PDF BÀN GIAO LÃNH ĐẠO**, giọng phi kỹ thuật — nguồn HTML regen ở `docs/pdf/src/`, cần mạng; ⚠️ PDF đang là bản 10/07, regen khi cần) — **PDF cho sếp/khách, .md cho dev**

> **Đã xóa/hợp nhất 11/07/2026** (chuẩn hóa 4 loại — git history còn): `docs/thiet-ke-ky-thuat-hop-nhat.md` · `docs/poc-zalo-bot.md` · `docs/poc-parser.md` → hợp nhất vào [docs/so-do-he-thong.md](docs/so-do-he-thong.md) (§2 + Phụ lục A/B) · `docs/tien-do-va-ke-hoach.md` · `docs/checklist-du-lieu-khach.md` · `docs/ke-hoach-dai-han.md` → tách/chuyển vào `docs/ke-hoach/` · `.Codex/plans/ultty-ai-agent.plan.md` + `.Codex/plans/phase3-nguon-su-that-dong.plan.md` (plan đã thực thi — phần dở nằm ở [docs/ke-hoach/nen-tang.md](docs/ke-hoach/nen-tang.md)).
...
| AI | Codex API (tool use) — **1 orchestrator điều phối 6 vai chuyên trách** (Điều phối · Tư vấn SP · Bán hàng · Chính sách-TC · Hậu mãi · Giám sát, theo `docs/khach-hang/ultty/nguon-goc/de-xuat-giai-phap-netviet.md` §5.1); **1 lần gọi LLM/tin** (Router parse), KHÔNG phải 6 LLM độc lập | Intent (7 loại) + trích xuất ràng buộc; **LLM không tính tiền/không quyết chính sách** — rules engine TS tất định lo phần đó; 6 vai hiển thị qua AgentTrace |
...
6. **Base dùng chung ⟂ gói khách (từ Đợt B1, 12/08/2026)** — hệ thống phục vụ NHIỀU khách (Ultty, Amico, …), nên `apps/` + `packages/` là NỀN TẢNG TRUNG TÍNH: **không được nhắc tên khách nào trong đó**. Mọi thứ chỉ đúng với một khách nằm ở `tenants/<slug>/` — `tenant.json` (danh tính + `persona.parserIntro`) và `data/knowledge.json` (SP/giá/đại lý/map nhóm/glossary), có zod schema, hỏng thì fail-fast lúc boot. Chọn khách: `TENANT=<slug>`, hoặc `TENANT_DIR=<path>` cho khách chạy hạ tầng riêng (mount gói ngoài image). Gói khách là **hạt giống**, không phải nguồn sự thật lúc chạy — với `PERSISTENCE=prisma` thì sau lần seed đầu Postgres mới là nguồn sự thật. Xem `tenants/README.md` + [docs/ke-hoach/nen-tang-da-khach.md](docs/ke-hoach/nen-tang-da-khach.md).
7. **Tích hợp ERP đi qua cổng `ErpPort`** (`apps/api/src/erp/erp.port.ts`), KiotViet chỉ là MỘT hiện thực (`KiotVietMockAdapter`). Khách khác dùng ERP khác (Amico: Nhanh.vn) thì thêm adapter + đổi một dòng `useClass`, KHÔNG sửa `orders.service`. Quyết định 1 ở trên vẫn giữ: không xây module kho riêng — ERP của khách là nguồn sự thật tồn kho.
...
- Dữ liệu khách hàng (SĐT, địa chỉ, đơn hàng) là dữ liệu nội bộ — **không gửi cho bên thứ 3** ngoài các API đã thống nhất (KiotViet, Codex API)
...
- **DeepSeek (`PARSER_MODE=deepseek`) + kênh zca (`CHANNEL_MODE=zca`) — lưu ý xử lý dữ liệu:** kênh zca đọc *mọi* tin nhóm rồi đẩy sang LLM. **DeepSeek CHƯA nằm trong danh sách bên thứ 3 được duyệt** (chỉ KiotViet + Codex). Do đó: (a) **demo chỉ dùng nhóm/dữ liệu TEST, không PII thật** — dùng DeepSeek ở giai đoạn này là chấp nhận được; (b) **bản chạy thật với dữ liệu khách** phải hoặc đổi sang **Codex** (`PARSER_MODE=Codex`), hoặc **bổ sung DeepSeek vào hợp đồng/thoả thuận xử lý dữ liệu** trước khi bật. Kèm điều kiện chặn của zca: tài khoản phụ + văn bản chấp nhận rủi ro ToS (mục "Kênh Zalo GĐ1").
